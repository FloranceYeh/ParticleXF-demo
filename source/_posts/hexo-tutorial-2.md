...
创建 `source/404.md` 文件，并在其中添加以下内容：

同时，在此文件内写的内容会在 404 页面中渲染。
...
{% note warning %}
当你是使用 `hexo server` 命令启动本地服务器时，访问不存在的页面会找不到 404 页面。
调试时要查看 404 页面，可以访问 `/404`。
生成静态文件或部署后，访问不存在的页面时就会显示 404 页面。
{% endnote %}
